...
class VectorRatio(nn.Module):
    def __init__(self, frames_number):
        ...
        self.multp = torch.autograd.Variable(
            torch.tensor(np.random.uniform(0, 1, frames_number)).reshape([frames_number, 1, 1, 1])).type(torch.FloatTensor)
        self.sig = nn.Sigmoid()
...
class FixedBlurLayer(nn.Module):
    def __init__(self, kernel):
        ...
        self.mask = nn.Parameter(data=torch.FloatTensor(self.mask_np), requires_grad=False)
