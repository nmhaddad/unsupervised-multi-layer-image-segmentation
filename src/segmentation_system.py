...
class SegmentationSystem(object):
    def __init__(self, image_name, image, plot_during_training=True,
                 ...
                 bg_hint=None, mg_hint=None, fg_hint=None,
                 show_every=500,
    ...
    def _init_nets(self):
        ...
        self.middle_net = middle_net.type(torch.FloatTensor)
    ...
    def _init_images(self):
        ...
        if self.fg_hint is not None:
            ...
            self.fg_hints = None
        if self.mg_hint is not None:
            assert self.mg_hint.shape[1:] == self.image.shape[1:]
            self.mg_hints = get_imresize_downsampled(self.mg_hint, downsampling_factor=self.downsampling_factor,
                                                     downsampling_number=self.downsampling_number)
            self.mg_hints_torch = [np_to_torch(mg_hint).type(torch.FloatTensor) for mg_hint in self.mg_hints]
        else:
            self.mg_hints = None
    ...
    def _init_noise(self):
        ...
        self.middle_net_inputs = self.left_net_inputs
    ...
    def _init_parameters(self):
        self.parameters = [p for p in self.left_net.parameters()] + \
                          [p for p in self.middle_net.parameters()] + \
    ...
    def finalize_first_step(self):
        ...
        middle = torch_to_np(self.middle_net_outputs[0])
        right = torch_to_np(self.right_net_outputs[0])
        save_image(self.image_name + "_1_left", left)
        save_image(self.image_name + "_1_middle", middle)
        ...
        save_image(self.image_name + "_hint3", self.mg_hint)
        ...
        save_image(self.image_name + "_hint3_masked", self.mg_hint * self.image)
    ...
    def finalize(self):
        ...
        save_image(self.image_name + "_middle", self.best_result.middle)
    ...
    def _initialize_any_step(self, iteration):
        ...
        for left_net_original_input, middle_net_original_input, right_net_original_input, mask_net_original_input \
                in zip(self.left_net_inputs, self.middle_net_inputs, self.right_net_inputs, self.mask_net_inputs):
            left_net_inputs.append(
                left_net_original_input + (left_net_original_input.clone().normal_() * reg_noise_std))
            middle_net_inputs.append(
                middle_net_original_input + (middle_net_original_input.clone().normal_() * reg_noise_std))
        ...
        self.middle_net_outputs = [self.middle_net(middle_net_input) for middle_net_input in middle_net_inputs]
    ...
    def _step1_optimization_closure(self, iteration):
        ...
        if self.fg_hints is not None and self.bg_hints is not None and self.mg_hints is not None:
    ...
    def _step1_optimize_with_hints(self, iteration):
        ...
        # add middleground loss
        self.total_loss += sum(self.extended_l1_loss(middle_net_output, image_torch, mg_hint) for
                               middle_net_output, mg_hint, image_torch
                               in zip(self.middle_net_outputs, self.mg_hints_torch, self.images_torch))
        # add mask loss TODO this needs to be multinomial distro
        self.total_loss += sum(self.l1_loss(((fg_hint - mg_hint - bg_hint) + 1) / 3, mask_net_output) for
                               fg_hint, mg_hint, bg_hint, mask_net_output in
                               zip(self.fg_hints_torch, self.mg_hints_torch, self.bg_hints_torch, self.mask_net_outputs))
        self.total_loss.backward(retain_graph=True)

    def _step2_optimize_without_hints(self, iteration):
        for left_out, middle_out, right_out, mask_out, original_image_torch in zip(self.left_net_outputs,
                                                                                   self.middle_net_outputs,
                                                                                   self.right_net_outputs,
                                                                                   self.mask_net_outputs,
                                                                                   self.images_torch):
            # TODO multinomial
            self.total_loss += 0.5 * self.l1_loss(mask_out * left_out + (1 - mask_out)/2 * right_out + (1 - mask_out)/2 * middle_out,
    ...
    def _step2_optimize_with_hints(self, iteration):
        ...
        for left_out, middle_out, right_out, mask_out, original_image_torch in zip(self.left_net_outputs,
                                                                                   ...
                                                                                   self.images_torch):
            # TODO multinomial
            self.total_loss += 0.5 * self.l1_loss(mask_out * left_out + (1 - mask_out)/2 * middle_out + (1 - mask_out)/2 * right_out,
                                                  original_image_torch)
            self.current_gradient = self.gray_loss(mask_out)
            # self.current_gradient = self.gradient_loss(mask_out)
    ...
    def _finalize_iteration(self):
        ...
        middle_out_np = torch_to_np(self.middle_net_outputs[0])
        ...
        # TODO multinomial
        self.current_psnr = peak_signal_noise_ratio(original_image, mask_out_np * left_out_np + (1 - mask_out_np)/2 * middle_out_np + (1 - mask_out_np)/2 * right_out_np)
    ...
    def _plot_with_name(self, name):
        if self.fg_hint is not None and self.bg_hint is not None and self.mg_hint is not None:
            plot_image_grid("left_right_hints_{}".format(name),
                            [np.clip(self.fg_hint, 0, 1),
                             np.clip(self.mg_hint, 0, 1),
                             np.clip(self.bg_hint, 0, 1)])

            for i, (left_out, middle_out, right_out, mask_out, image) in enumerate(zip(self.left_net_outputs,
                                                                                       self.middle_net_outputs,
                                                                                       self.right_net_outputs,
                                                                                       self.mask_net_outputs,
                                                                                       self.images)):

                plot_image_grid("left_middle_right_{}_{}".format(name, i),
                                [np.clip(torch_to_np(left_out), 0, 1),
                                 np.clip(torch_to_np(middle_out), 0, 1),
                ...
                plot_image_grid("learned_image_{}_{}".format(name, i),
                                [np.clip(
                                    # TODO multinomial
                                    mask_out_np * torch_to_np(left_out) + (1 - mask_out_np)/2 * torch_to_np(middle_out) + (1 - mask_out_np)/2 * torch_to_np(right_out),
                                    0, 1), image])

def image_segmentation(image_name, image, plot_during_training=True, first_step_iter_num=4000, second_step_iter_num=8000,
                  bg_hint=None, fg_hint=None, mg_hint=None, show_every=500, downsampling_factor=0.1, downsampling_number=0):

    segment = SegmentationSystem(image_name, image, plot_during_training=plot_during_training,
                                 first_step_iter_num=first_step_iter_num, second_step_iter_num=second_step_iter_num,
                                 bg_hint=bg_hint, fg_hint=fg_hint, mg_hint=mg_hint, show_every=show_every,
                                 downsampling_factor=downsampling_factor, downsampling_number=downsampling_number)
    segment.optimize()
    segment.finalize()


def triple_dip_segmentation(input_name, output_name, fg_hint, bg_hint, mg_hint,
                            plot_during_training=True, first_step_iter_num=4000, second_step_iter_num=8000,
                            show_every=500, downsampling_factor=0.1, downsampling_number=0):
    """
    ...
    :param bg_hint:
    :param mg_hint:
    :param plot_during_training:
    :param first_step_iter_num:
    :param second_step_iter_num:
    :param show_every:
    :param downsampling_factor:
    :param downsampling_number:
    :return:
    """
    runpy.run_module("bg_fg_mg_prep", run_name="__main__")

    img = prepare_image("input/images/segmentation/" + input_name)
    fg = prepare_image("input/images/segmentation_" + fg_hint + "/" + input_name)
    ...
    image_segmentation("images/segmentation/" + output_name, img, plot_during_training=plot_during_training,
                       ...
                       bg_hint=bg, fg_hint=fg, mg_hint=mg,
